<?php
namespace common\components\utils\php\pg;
/**
...
class PHPG {
	...
	public function __construct($connection_alias, $connection_params = Null) {
		...
		if($connection_params === Null) {
			...
			if(array_key_exists($connection_alias, $GLOBALS['PHPG'])) { // Connection alias exists
				...
				throw new \Exception('Connection alias specified "' . $connection_alias . '" does not exists');
			}
		}
		...
		if($connection_datatype == 'string') {
			...
			throw new \Exception('Expecting connection_params to be string or array, encountered "' . $connection_datatype . '"');
		}
	...
	private function _connect($connection_string) {
		...
		if($connection === False) {
			throw new \Exception('Unable to connect to PostgreSQL database with error: ' . pg_last_error());
		}
	...
	public function commit() {
		...
		if($commit_result === False) {
			...
			throw new \Exception('Error while attempting commit: ' . $commit_error);
		}
...
class PHPG_Cursor {
	...
	public function execute($query) {
		...
		if($resource === False) {
			throw new \Exception('Error with query: ' . pg_last_error($GLOBALS['PHPG'][$this->connection_alias]['connection']));
		}
	...
	*                                                                       *
	...
	private function _transform_json($value, $data_type) {
		...
		if($json_error === JSON_ERROR_NONE) {
			...
			throw new \Exception('Error while decoding JSON data type: ' . $json_error);
		}
