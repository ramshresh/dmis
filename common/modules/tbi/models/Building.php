...
class Building extends \yii\db\ActiveRecord
{
    ...
    public function prepareDownloadGalleryImages($targetDirectory)
    {
        /* @var $behavior \ramshresh\yii2\galleryManager\GalleryBehavior */
        $behavior = $this->getBehavior('buildingPhotoGalleryBehavior');
        $images = $this->galleryImages;
        foreach ($images as $image) {
            $behavior->copyFiles($image->id,$targetDirectory);
        }
 ...
 public  function chmod_r($dir, $dirPermissions, $filePermissions) {
    ...
    public function zipPhotos($zipName)
    {
        $images = $this->galleryImages;
        foreach ($images as $image) {
            /* @var $behavior \ramshresh\yii2\galleryManager\GalleryBehavior */
            $behavior = $this->getBehavior('buildingPhotoGalleryBehavior');
            $behavior->exportToZip($image->id,$zipName);
        }
    }
