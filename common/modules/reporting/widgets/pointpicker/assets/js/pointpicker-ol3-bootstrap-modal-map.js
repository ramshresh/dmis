...
(function ($, window, document, undefined) {
    var PointPickerMap = {
        ...
        initOpenLayersMap: function () {
            ...
            if (ppM.externalMapDivId) {	// get the external map instance
                ...
                // Create a map
                console.log('making map at <div id="'+ppM.mapDivId+'">');
                console.log($('#ppM.mapDivId'));
                map = new ol.Map({
                    target: ppM.mapDivId,
                    layers: [
                        new ol.layer.Tile({
    ...
    var PointPicker = {
        ...
        activateTrigger: function () {
            ...
            $(pointPicker.$iTriggerDialog).click(function () {
				if ($(pointPicker.$iDialog).dialog("isOpen") == false) {
        ...
        initHTMLElements: function () {
            var pointPicker = this;
            pointPicker.$iTriggerDialog=$('#' + pointPicker.triggerId);
            pointPicker.$iDialog = $(pointPicker.elem).children('.pointpickerdialog')[0];
            $('#'+pointPicker.widgetDivId).dialog({autoOpen: false});

            pointPicker.$iDialog=$('#'+pointPicker.widgetDivId);
			/*
			$(pointPicker.$iDialog).css({display: 'none'});
			$(pointPicker.$iDialog).dialog({
				autoOpen: false,
